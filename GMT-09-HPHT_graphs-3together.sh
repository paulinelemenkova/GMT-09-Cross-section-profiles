...
# Step-1. GMT set up
gmt set FONT_ANNOT_PRIMARY=9p,Palatino-Roman,dimgray \
...
gmt psxy -R-200/200/-7000/0 -JX15.2c/5 envH.txt \
    -Bpxa50g50f10+l"Distance from trench (km)"\
    -Bpya2000gf+l"Depth (m)" \
    -Bsxg50 -Bsyg1000 -BWeSn \
...
echo "110 -5000 Pacific Plate" | gmt pstext -R -J \
    -Gwhite -F+jBL+f11p,brown -O -K >> $ps
echo "-10 -1000 Hjort Trench" | gmt pstext -R -J \
    -Gwhite -F+jBL+f12p,navyblue -O -K >> $ps
echo "90 -5000 Modelled median stacked" | gmt pstext -R -J \
    -Gwhite -F+jBL+f11p,Times-Roman,red -O -K >> $ps
echo "90 -5700 cross-section profiles" | gmt pstext -R -J \
    -Gwhite -F+jBL+f11p,Times-Roman,red -O -K >> $ps
echo "90 -6400 with error bars" | gmt pstext -R -J \
    -Gwhite -F+jBL+f11p,Times-Roman,red -O -K >> $ps
echo "-190 -6200 C" | gmt pstext -R -J -F+jBL+f18p,black \
    -Gfloralwhite -W0.5p -O -K >> $ps
# Step-17. Arrow
...
echo "-190 -2400 with error bars" | gmt pstext -R -J \
    -Gwhite -F+jBL+f11p,Times-Roman,red -O -K >> $ps
echo "110 -6000 Pacific Plate" | gmt pstext -R -J \
    -Gwhite -F+jBL+f11p,brown -O -K >> $ps
echo "56 0 Puysegur Ridge" | gmt pstext -R -J \
    -Gwhite -F+jBL+f11p,brown -O -K >> $ps
echo "-50 0 Puysegur Trench" | gmt pstext -R -J \
    -Gwhite -F+jBL+f12p,navyblue -O -K >> $ps
echo "-190 -6200 B" | gmt pstext -R -J -F+jBL+f18p,black \
    -Gfloralwhite -W0.5p -O -K >> $ps
# Step-17. Arrow
gmt psxy -R -J -Sv0.15i+bc+ea -Gyellow -W0.5p -O -K << EOF >> $ps
0 -500 270 1.7c
...
echo "110 -6000 Pacific Plate" | gmt pstext -R -J \
    -Gwhite -F+jBL+f11p,brown -O -K >> $ps
echo "100 -1000 North Island" | gmt pstext -R -J \
    -Gwhite -F+jBL+f11p,brown -O -K >> $ps
echo "-40 500 Hikurangi Trench" | gmt pstext -R -J \
    -Gwhite -F+jBL+f12p,navyblue -O -K >> $ps
echo "-190 -3800 A" | gmt pstext -R -J -F+jBL+f18p,black \
    -Gfloralwhite -W0.5p -O -K >> $ps
# Step-17. Arrow
gmt psxy -R -J -Sv0.15i+bc+ea -Gyellow -W0.5p -O -K << EOF >> $ps
0 0 270 1.7c
