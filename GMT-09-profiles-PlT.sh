...
# Area: along the track of the Pliny Trench
...
gmt grdcontour ht_relief1.nc -R -J -C1000 \
    -B+t"Cross-sectional profiles of the Pliny Trench, Mediterranean Sea. DEM: GEBCO" \
    --MAP_TITLE_OFFSET=0.5c \
    -W0.1p -O -K >> $ps
...
cat << EOF > trenchPlT.txt
25.3 34.2
26.9 34.8
EOF

# Step-12. Plot trench segment and end points
gmt psxy -R -J -W2p,red trenchPlT.txt -O -K >> $ps # my line
gmt psxy -R -J -Sc0.15i -Gred trenchPlT.txt -O -K >> $ps # points
...
gmt grdtrack trenchPlT.txt -Ght_relief1.nc -C200k/2k/10k+v -Sm+sstackPlT.txt > tablePlT.txt
gmt psxy -R -J -W0.5p,white tablePlT.txt -O -K >> $ps

# Show upper/lower values encountered as an envelope
gmt convert stackPlT.txt -o0,5 > envPlT.txt
gmt convert stackPlT.txt -o0,6 -I -T >> envPlT.txt

# Plot graph (statistical mean for the profiles)
gmt psxy -R-100/100/-5000/2000 -JX15.2c/5c -Y13.0c envPlT.txt -W0.5p \
...
gmt psxy -R -J -W1.0p -Ey+p0.2p stackPlT.txt -O -K >> $ps
gmt psxy -R -J -W1.0p,red stackPlT.txt -O -K >> $ps

# Add test annotations
echo "-90 -900 Median stacked profile with error bars" | gmt pstext -R -J -Gwhite -F+jBL+f10p,Helvetica,red -O -K >> $ps
echo "51 -4700 Aegean Sea Plate " | gmt pstext -R -J -F+jBL+f12p,Helvetica,orangered4 -Gwhite -O -K >> $ps
echo "-80 -4700 African Plate " | gmt pstext -R -J -F+jBL+f12p,Helvetica,orangered4 -Gwhite -O -K >> $ps
echo "-15 500 Pliny Trench" | gmt pstext -R -J -F+f12p,Helvetica,orangered4+jBL -Gwhite -O -K >> $ps
echo "80 -2400 Crete" | gmt pstext -R -J -F+jBL+f11p,orangered4 -Gwhite -O -K >> $ps
echo "80 -3200 Island" | gmt pstext -R -J -F+jBL+f11p,orangered4 -Gwhite -O -K >> $ps
echo "-95 -4500 A" | gmt pstext -R -J -F+jBL+f18p,black -Gfloralwhite -W0.5p -O -K >> $ps
...
gmt logo -Dx6.4/-14.8+0.25c/1.5c+w2c -O >> $ps
...
gmt psconvert crossPlT.ps -A0.2c -E720 -Tj -P -Z
